...
int Astar::search(const Eigen::Vector3d& start_pt, const Eigen::Vector3d& end_pt) {
  ...
  while (!open_set_.empty()) {
    ...
    if ((ros::Time::now() - t1).toSec() > max_search_time_) {
      // std::cout << "early" << endl;
      // cout << cur_node->index(0) << " " << cur_node->index(1) << " " << cur_node->index(2) << endl;
      // cout << end_index(0) << " " << end_index(1) << " " << end_index(2) << endl;
      // cout << "time up" << endl;
...
int Astar::search_evpp( const Eigen::Vector3d& start_pt, const Eigen::Vector3d& end_pt, 
    ...
    while (!open_set_.empty()) {
      ...
      if ((ros::Time::now() - t1).toSec() > max_search_time_) {
        ...
        // cout << end_index(0) << " " << end_index(1) << " " << end_index(2) << endl;
        cout << "time up" << endl;
        early_terminate_cost_ = cur_node->g_score + getEuclHeu(cur_node->position, end_pt);
        return NO_PATH;
      }
      ...
      for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
        for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
          for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
            ...
            // 更新 nbr_pos 节点的信息
            double dis = len;
            node_info_map[nbr_pos].path_length = node_info_map[cur_pos].path_length + dis;
            node_info_map[nbr_pos].total_gain = node_info_map[cur_pos].total_gain + get_gain(nbr_pos);
            node_info_map[nbr_pos].num_points = node_info_map[cur_pos].num_points + 1;

            neighbor->parent = cur_node;
            neighbor->g_score = tmp_g_score;
            double ave_gain = node_info_map[nbr_pos].path_length*node_info_map[nbr_pos].total_gain/node_info_map[nbr_pos].num_points;
            // cout << "tmp_g_score = " << tmp_g_score << " ave_gain = " << ave_gain << endl;
            // cout << "lambda_heu_ = " << lambda_heu_ << endl;
            // cout << "getEuclHeu(nbr_pos, end_pt)= " << getEuclHeu(nbr_pos, end_pt) << endl;
            neighbor->f_score = tmp_g_score + lambda_heu_ * getEuclHeu(nbr_pos, end_pt) - lamda*ave_gain;
            open_set_.push(neighbor);
            open_set_map_[nbr_idx] = neighbor;
          }
    }
