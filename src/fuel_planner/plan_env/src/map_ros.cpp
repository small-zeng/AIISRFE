...
void MapROS::init() {
  ...
  node_.param("map_ros/show_occ_time", show_occ_time_, true);
  node_.param("map_ros/show_esdf_time", show_esdf_time_, false);
  node_.param("map_ros/show_all_map", show_all_map_, false);
  node_.param("map_ros/frame_id", frame_id_, string("world"));

  cout << "K = " << fx_ << fy_ << cx_  << cy_  << endl;
  int W = (int)(2*cx_ +1);
  int H = (int)(2*cy_ +1);
  proj_points_.resize((W * H) / (skip_pixel_ * skip_pixel_));
  point_cloud_.points.resize(W * H / (skip_pixel_ * skip_pixel_));
...
void MapROS::depthPoseCallback(const sensor_msgs::ImageConstPtr& img,
  ...
  img_num_ += 1;
  // ROS_WARN("[map_ros]img_num_: %d", img_num_);
...
void MapROS::proessDepthImage() {
  ...
  // cout << "camera_p = " << camera_pos_ << endl;

  Eigen::Vector3d pt_cur_tmp, pt_cur, pt_world;
  ...
  for (int v = depth_filter_margin_; v < rows - depth_filter_margin_; v += skip_pixel_) {
    ...
    for (int u = depth_filter_margin_; u < cols - depth_filter_margin_; u += skip_pixel_) {
      ...
      pt_cur_tmp(0) = (u - cx_) * depth / fx_;
      pt_cur_tmp(1) = (v - cy_) * depth / fy_;
      pt_cur_tmp(2) = depth;
      pt_cur << pt_cur_tmp(0),-pt_cur_tmp(1),-pt_cur_tmp(2);
...
void MapROS::publishMapAll() {
  ...
  ofstream file("/mnt/dataset/zengjing/monosdf_planning/src/fuel_planner/exploration_manager/resource/curve2.txt",ios::app);
...
void MapROS::publishESDF() {
  ...
  // cout << "map_->md_->local_bound_min_ = " << map_->md_->local_bound_min_.transpose() << endl;
  // cout << "map_->md_->local_bound_max_ = " << map_->md_->local_bound_max_.transpose() << endl;
  // cout << "map_->mp_->local_map_margin_ = " << map_->mp_->local_map_margin_ << endl;
  ...
  // cout << "max_cut = " << max_cut.transpose() << endl;
  map_->boundIndex(min_cut);
  map_->boundIndex(max_cut);
  ...
  for (int x = min_cut(0); x <= max_cut(0); ++x)
    for (int z = min_cut(2); z <= max_cut(2); ++z) {
      Eigen::Vector3d pos;
      map_->indexToPos(Eigen::Vector3i(x, 1, z), pos);
      pos(1) = esdf_slice_height_;
