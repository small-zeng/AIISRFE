...
plannerResult FastExplorationManager::planExploreMotion_REC(
  ...
  if (ed_->points_.size() > 1) {
    ...
    int knum = min(int(indices.size()), 1);
    // Choose the next viewpoint from global tour
    nbv_pos = ed_->points_[indices[0]];
    nbv_pitch = ed_->pitchs_[indices[0]];
    nbv_yaw = ed_->yaws_[indices[0]];
  
  }else if (ed_->points_.size() == 1) {
    // Only 1 destination, no need to find global tour through TSP
    nbv_pos = ed_->points_[0];
    nbv_pitch = ed_->pitchs_[0];
    nbv_yaw = ed_->yaws_[0];
  } else {
  ...
  cout << "[EXP] NBV = \n" ;
  cout << "nbv_pos = " << nbv_pos.transpose() << endl;
  cout << "nbv_pitch = " << nbv_pitch << endl;
  cout << "nbv_yaw = " << nbv_yaw << endl;
  ...
  // add nbv to next_poses
  next_poses.push_back(nbv_pos);
  next_pitchs.push_back(nbv_pitch);
  next_yaws.push_back(nbv_yaw);
  ...
  cout << endl;
  result = processViewPath_REC(cur_pos,cur_pitch,cur_yaw, ed_->all_points_,ed_->all_pitchs_,ed_->all_yaws_);
...
plannerResult FastExplorationManager::processViewPath_REC(const Vector3d & pos, const double & pitch, const double & yaw, 
     ...
     // process view and path
    int SURF_NUM = 0,SURF_NUM_MAX = 20, n = 20;
...
void FastExplorationManager::getFullCostMatrix_All(const Vector3d& cur_pos, const double& cur_yaw, const double& cur_pitch,
    ...
    for (int i = 0; i < next_poses.size(); ++i) {
      for (int j = 1; j < next_poses.size(); ++j) {
