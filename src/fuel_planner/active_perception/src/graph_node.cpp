...
ViewNode::ViewNode(const Vector3d& p, const double& pitch, const double& yaw) {
  pos_ = p;
  pitch_ = pitch;
  yaw_ = yaw;
  parent_ = nullptr;
...
double ViewNode::computeCost(const Vector3d& p1, const Vector3d& p2, const double& y1, const double& y2,
      ...
      double path_length = ViewNode::searchPath(p1, p2, path) ;

      if (path_length == 5000)
      {
        path.clear();
      }
      double pos_cost = path_length / vm_;
      return pos_cost;
...
double ViewNode::computeCostNew(const Vector3d& p1, const Vector3d& p2, const double& yaw1, const double& yaw2,
                             const double& pitch1, const double& pitch2, vector<Vector3d>& path){
      // Cost of position change
      ViewNode::astar_->max_search_time_ = 0.004;
      double path_length = ViewNode::searchPath(p1, p2, path) ;
