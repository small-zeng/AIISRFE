...
from core.model.embedder import *
from core.model.density import LaplaceDensity
from core.model.ray_sampler import ErrorBoundSampler
...
from core.hashencoder.hashgrid import _hash_encode, HashEncoder
...
class RenderingNetwork(nn.Module):
    ...
    def forward(self, points, normals, view_dirs, feature_vectors, indices):
        ...
        # view_dirs[:,1] = 0
        # view_dirs[:,2] = 1
        # print("view_dirs before embed", view_dirs.shape)
        if self.embedview_fn is not None:
            view_dirs = self.embedview_fn(view_dirs)
        # print("view_dirs after embed", view_dirs.shape)
...
class MonoSDFNetwork(nn.Module):
    def __init__(self, conf):
        ...
        # self.bg_color = torch.tensor(conf.get_list("bg_color", default=[240./255.,183./255.,233./255.])).float().to(device)
    ...
    def forward(self, input, indices,device=device,is_uncer_only=False):
        ...
        rgb_flat, uncertainty = self.rendering_network(points_flat, gradients, dirs_flat, feature_vectors, indices)
        rgb = rgb_flat.reshape(-1, N_samples, 3)
        # print(uncertainty.shape)
        uncer_map = uncertainty.reshape(-1, N_samples)
        uncer_map = torch.sum(uncer_map,-1)
        
        # print("t1-3 =",time.time()-t0)
        ...
        if self.training:
            ...
            batch_size,num_pixels = 1, ray_dirs.shape[0]
        ...
        normal_map =torch.matmul(rot,normal_map)[:,:,0]
        normal_map = normal_map.contiguous()
          

        ## Modified by zengjing  Blender 
        ## Blender
