...
class UniformSampler(RaySampler):
    ...
    # dtu and bmvs
    def get_z_vals_dtu_bmvs(self, ray_dirs, cam_loc, model,device=device):
        if not self.take_sphere_intersection:
            near, far = self.near * torch.ones(ray_dirs.shape[0], 1).to(device), self.far * torch.ones(ray_dirs.shape[0], 1).to(device)
        else:
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model,device=device):
        if not self.take_sphere_intersection:
            near, far = self.near * torch.ones(ray_dirs.shape[0], 1).to(device), self.far * torch.ones(ray_dirs.shape[0], 1).to(device)
        else:
...
class ErrorBoundSampler(RaySampler):
    ...
    def get_z_vals(self, ray_dirs, cam_loc, model,device=device):
        ...
        z_vals, near, far = self.uniform_sampler.get_z_vals(ray_dirs, cam_loc, model,device=device)
        ...
        while not_converge and total_iters < self.max_total_iters:
            ...
            d_star = torch.zeros(z_vals.shape[0], z_vals.shape[1] - 1).to(device)
            ...
            curr_error = self.get_error_bound(beta0, model, sdf, z_vals, dists, d_star,device=device)
            ...
            for j in range(self.beta_iters):
                ...
                curr_error = self.get_error_bound(beta_mid.unsqueeze(-1), model, sdf, z_vals, dists, d_star,device=device)
            ...
            shifted_free_energy = torch.cat([torch.zeros(dists.shape[0], 1).to(device), free_energy[:, :-1]], dim=-1)
            ...
            if not_converge and total_iters < self.max_total_iters:
                ...
                bins = z_vals
                error_per_section = torch.exp(-d_star / beta.unsqueeze(-1).clone()).clone() * (dists[:,:-1] ** 2.) / (4 * beta.unsqueeze(-1).clone() ** 2)
            ...
            if (not_converge and total_iters < self.max_total_iters) or (not model.training):
                u = torch.linspace(0., 1., steps=N).to(device).unsqueeze(0).repeat(cdf.shape[0], 1)
            else:
        ...
        if self.inverse_sphere_bg: # if inverse sphere then need to add the far sphere intersection
            far = rend_util.get_sphere_intersections(cam_loc, ray_dirs, r=self.scene_bounding_sphere,device=device)[:,1:]
        ...
        idx = torch.randint(z_vals.shape[-1], (z_vals.shape[0],)).to(device)
    ...
    def get_error_bound(self, beta, model, sdf, z_vals, dists, d_star,device=device):
        density = model.density(sdf.reshape(z_vals.shape), beta=beta)
        shifted_free_energy = torch.cat([torch.zeros(dists.shape[0], 1).to(device), dists * density[:, :-1]], dim=-1)
