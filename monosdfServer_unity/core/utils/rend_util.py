...
def get_psnr(img1, img2, normalize_rgb=False,device=device):
...
def load_rgb(path, img_res, normalize_rgb = False):
    img = imageio.imread(path)
    img = skimage.img_as_float32(img)
    img = cv2.resize(img, (img_res[1],img_res[0]), interpolation=cv2.INTER_AREA)
...
def load_depth(path, img_res,normalize_rgb = False,l435 = False):
    img = imageio.imread(path)
    img = cv2.resize(img, (img_res[1],img_res[0]), interpolation=cv2.INTER_AREA)
    if l435:
        img = img[...,None]
...
def get_camera_params(uv, pose, intrinsics,device=device):
    ...
    pixel_points_cam = lift(x_cam, y_cam, z_cam, intrinsics=intrinsics,device=device)
...
def lift(x, y, z, intrinsics,device=device):
    # parse intrinsics
    ...
    # print(fx,fy,cx,cy,sk)
    x_lift = (x - cx.unsqueeze(-1) + cy.unsqueeze(-1)*sk.unsqueeze(-1)/fy.unsqueeze(-1) - sk.unsqueeze(-1)*y/fy.unsqueeze(-1)) / fx.unsqueeze(-1) * z
    y_lift = (y - cy.unsqueeze(-1)) / fy.unsqueeze(-1) * z
    
    # Modified by zengjing Blender / Colmap / Unity / L435
    return torch.stack((x_lift, -y_lift, -z, torch.ones_like(z).to(device)), dim=-1)
    # return torch.stack((x_lift, y_lift, z, torch.ones_like(z).to(device)), dim=-1)
    # return torch.stack((x_lift, -y_lift, -z, torch.ones_like(z).to(device)), dim=-1)
    # return torch.stack((z, -x_lift, -y_lift, torch.ones_like(z).to(device)), dim=-1)


def quat_to_rot(q,device=device):
...
def rot_to_quat(R,device=device):
...
def get_sphere_intersections(cam_loc, ray_directions, r = 1.0,device=device):
