...
from render.utils.general import trans_topil
...
def plot_test(model, indices, plot_data, path, epoch, img_res, plot_nimgs, resolution, grid_boundary,  level=0,data_type = "train", is_vs = True,device=device):
    ...
    if plot_data is not None:
        ...
        print(plot_data['rgb_eval'].shape,plot_data['rgb_gt'].shape,device)
        ...
        with open(filename,'a') as f: 
            f.write("current step is + " + str(epoch)+"\n")
        ...
        for name in ["rendering", "depth", "normal"]:
            images.append(cv2.imread('{0}/{1}_{2}.png'.format(path, name, indices[0])))        

        images = np.concatenate(images, axis=1)
        cv2.imwrite('{0}/merge_{1}.png'.format(path, indices[0]), images)
    ...
    if indices[0]==0:
        ...
        print("time for surface tracing is ", time.time()-ts)
...
#@torch.no_grad()
def get_surface_sliding(path, indices, epoch, model, sdf, resolution=100, grid_boundary=[-2.0, 2.0], return_mesh=False, level=0,device=device):
    ...
    scale_mat = np.loadtxt("../data/scale_mat/hm3d_00804.txt") 
    ...
    if return_mesh:
        ...
        combined.export('{0}/surface512rgb_{1}.ply'.format(path, epoch), 'ply') 
...
def plot_normal_maps(normal_maps, ground_true, path, epoch, plot_nrow, img_res, indices, is_vs = True,device=device):
    ...
    img.save('{0}/normal_{1}.png'.format(path, indices[0]))
...
def plot_images(rgb_points, ground_true, path, epoch, plot_nrow, img_res, indices, exposure=False, is_vs = True,device=device):
    ...
    img = Image.fromarray(tensor)
    if exposure:
        img.save('{0}/exposure_{1}.png'.format(path, indices[0]))
    else:
        img.save('{0}/rendering_{1}.png'.format(path, indices[0]))
...
def plot_depth_maps(depth_maps, ground_true, path, epoch, plot_nrow, img_res, indices, is_vs = True,device=device):
    ...
    save_path = '{0}/depth_{1}.png'.format(path, indices[0])
