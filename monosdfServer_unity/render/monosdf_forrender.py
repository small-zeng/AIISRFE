...
import render.utils.plots as plt
import render.utils.general as utils
from render.common import *
from render.model.loss import compute_scale_and_shift
from render.utils.general import BackprojectDepth
...
## hm3d
base_dir = "/mnt/dataset/zengjing/monosdf_planning"
exps_folder = "exps"
expname = "hm3d_im1_grids_1"
scene_name = 'test_10_2_6'
version = ''
plot_dir = "plots"
...
class Controller():
    ...
    def __init__(self, H, W, focal, imgs, depth_imgs,normal_imgs, poses):
        ...
        self.exps_folder_name = exps_folder
        ...
        if self.GPU_INDEX == 0 and  is_continue:
            
            utils.mkdir_ifnotexists(os.path.join(base_dir,self.exps_folder_name))
            self.expdir = os.path.join(base_dir, self.exps_folder_name, self.expname)
            utils.mkdir_ifnotexists(self.expdir)
            # self.timestamp = '{:%Y_%m_%d_%H_%M_%S}'.format(datetime.now())
            self.timestamp = scene_name + version
            utils.mkdir_ifnotexists(os.path.join(self.expdir, self.timestamp))
            
            self.plots_dir = os.path.join(self.expdir, self.timestamp, plot_dir)
    ...
    def load_model(self,checkpoint):
        ...
        data = torch.load(
            os.path.join(old_checkpnts_dir, self.scheduler_params_subdir, str(checkpoint) + ".pth"))
        self.scheduler.load_state_dict(data["scheduler_state_dict"])
    ...
    def info_planner(self):
        num = self.add_img_index
        print("add_img_set num = ", num)
        if num >= 40 or self.NBV_step > 50:
            return
        ...
        self.model.train()
        response = requests.get("http://10.181.108.29:20049/isfinish", "finish=yes")
    ...
    def train(self):
        ...
        while True:
            ...
            if (self.iter_step % 700 == 0 and self.iter_step > 300 and self.iter_step < 25000) :
                torch.cuda.empty_cache()
                # threading.Thread(target=self.info_planner,args=()).start
                self.info_planner()
                
            # if  self.iter_step == 200:
            #     torch.cuda.empty_cache()
            #     # threading.Thread(target=self.info_planner,args=()).start
            #     self.info_planner()
            ...
            if self.iter_step % 4 == 0 and self.iter_step < (80*self.add_img_index):
                # n_index = np.random.randint(self.add_img_index,size=1)[0]
                n_index = currindex
            else :
                # n_index = (self.iter_step) % self.add_img_index
                n_index = np.random.randint(self.add_img_index,size=1)[0]
    ...
    def get_all_uncertainty(self,pose,epoch,data_type = "train",index = 0,device=device):
        self.train_dataset.change_sampling_idx(-1)
        indices, model_input, ground_truth = self.train_dataset.get_testdata(index,data_type = data_type)
        print(indices)
        model_input["intrinsics"] = model_input["intrinsics"].to(device)
        ...
        model_input['pose'] = torch.from_numpy(pose[np.newaxis,...]).float().to(device)
        sample = self.get_sampler(model_input, device=device)
    ...
    def get_plot_data(self, model_input, model_outputs, pose, rgb_gt, normal_gt, depth_gt,device=device):
        ...
        # depth_map_cpu = depth_map.cpu().numpy().reshape((384,384))
        # print("depth_map = ",depth_map_cpu)
        # print(scale,shift)
...
def config_parser():
    ...
    parser.add_argument('--conf', type=str, default='hm3d_00804_im_grids.conf')
    ...
    parser.add_argument('--timestamp', default='test_21_6_5', type=str,
