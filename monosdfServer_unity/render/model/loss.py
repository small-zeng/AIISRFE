...
class ScaleAndShiftInvariantLoss(nn.Module):
    ...
    def forward(self, prediction, target, mask):
        ...
        # print("depth_loss 1 : ", total)
        if self.__alpha > 0:
            gradient_loss = self.__regularization_loss(self.__prediction_ssi, target, mask)
            total += self.__alpha * gradient_loss
            # print("depth_loss 2 : ", gradient_loss)
...
class MonoSDFLoss(nn.Module):
    ...
    def forward(self, model_outputs, ground_truth,device=device):
        rgb_gt = ground_truth['rgb'].to(device)
        # monocular depth and normal
        depth_gt = ground_truth['depth'].to(device)
        normal_gt = ground_truth['normal'].to(device)
        ...
        # print(model_outputs,ground_truth)
        # print(output)
...
class MonoSDFIMLoss(nn.Module):
    ...
    def get_depth_loss(self, depth_pred, depth_gt, mask):
        # TODO remove hard-coded scaling for depth
        # return self.depth_loss(depth_pred.reshape(1, 64, 64), (depth_gt * 50 + 0.5).reshape(1, 64, 64), mask.reshape(1, 64, 64))
        # Modified by zengjing      Others / L435
        return self.depth_loss(depth_pred.reshape(1, 32, 32), (depth_gt * 50 + 0.5).reshape(1, 32, 32), mask.reshape(1, 32, 32))
    ...
    def forward(self, model_outputs, ground_truth,device=device):
        ...
        # print(model_outputs['rgb_values'])
        rgb_loss = self.get_rgb_loss(model_outputs['rgb_values'], rgb_gt)


        delta = torch.mean(model_outputs['uncer_map'])
        # print("delta = ",delta)
        # # rgb_loss = rgb_loss *10
        rgb_loss = 1/2*torch.log(delta)+1/2*rgb_loss/delta
        rgb_loss = rgb_loss *0.03
        ...
        # combine with GT
        mask = (ground_truth['mask'] > 0.5).to(device) & mask
        # mask = (depth_gt > 0.0).to(device) & mask
        # mask = (depth_gt < 6.0).to(device) & mask
